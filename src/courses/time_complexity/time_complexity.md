...
M1kado (連子賢), Minor Revise By D1stance
~~~
...
定義: 若 $T(n) \in O(g(n))$ 若且唯若存在一 $c$, $n_0$
使得當 $n \ge n_0$ 時 $0 \le T(n) \le c \cdot g(n)$

用白話文來說，就是可以找到另外一個函數 $g(n)$ 在某個數值後都比 $f(n)$ 大，我們就可以用 $g(n)$ 來當作 $f(n)$ 的上限。
...
舉例:
- $T(n) = 5 n^2 + 2n + 1 \in O(n^2)$
- $T(n) = 2n + 2^n \in O(2^n)$
- $T(n, m) = 2n + m^2 + 3m \in O(m^2 + n)$
- $T(n) = \log n + \sqrt n \in O(\sqrt n)$ 
因為 $\sqrt n$ 增長較快。

在估計時間複雜度時，通常取 Big-$O$ 最精確的值，
例如 $T(n) = n$ 的情況下，我們會說 $O(n)$ 而非 $O(n^2)$，因為後者比較不精確，
其實也就是數學上 Big-$\Theta$ 的概念，但這裡不多做介紹。
...
因此需要尋找更優的演算法，同時在題目中如果看到比較特別的數據範圍，也可以想一下有沒有可以利用的地方。
...
所以時間複雜度是 $T(n) = [n + (n - 1) + (n - 2) + ... + 1] \in O(n^2)$
...
所以時間複雜度是 $T(n) = [(n - 1) + (n - 2) + ... + 1] \in O(n^2)$
...
對於排序一個陣列或 vector，只要傳入起始位置和結束位置 **後一格** 的位置即可 (左閉右開)。
...
它跟指標有點不太一樣，在這裡不深入細談，你只要知道它們所代表的意義即可，我們在之後的章節會深入細談。
