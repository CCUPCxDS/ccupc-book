...
定義長度為 $\pi(i)$，直接找太慢了，這樣是 $O(|T|^2)$，我們需要更有效率的作法。
...
所以我們可以先看 "aba" 的最長配對在哪，"aba" 最長配對是 "a" 所以 $\pi(2) = 1$，
剛好最長配對的下一個字元就是 'b'，所以 $\pi(3) = \pi(2) + 1 = 2$。

那麼 "ababd" 呢？ 因為 'd' != "abab" 之最長配對的下字元 'a'，那怎麼辦？
...
"ababa" 的前綴有 "a", "ab", "aba", "abab"
後綴有 "a", "ba", "aba", "baba"，所以 $\pi(4) = 3$，
但是新加進來的 'b' 無法在最長配對後面，
所以我們考慮次長，發現可以接在次長配對 "a" 後面，所以 $\pi(5) = 2$，
同時也可以觀察發現 次長配對 "a" = 最長配對 "aba" 的最長配對，
所以我們可以用這個想法去加速計算。
